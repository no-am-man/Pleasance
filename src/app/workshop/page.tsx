                ...
                <div className="flex -space-x-2 overflow-hidden">
                    <TooltipProvider>
                     ...
                     {(!presentUsers || presentUsers.length === 0) && (
                        <span className="text-sm text-muted-foreground self-center">You're the only one here.</span>
                    )}
                </div>
            </CardHeader>
        </Card>
...
const convertPixelsToObj = (pixels: ColorPixel[]): string => {
    let objContent = '# 3D Point Cloud generated by Pleasance AI Workshop\n';
...
function SaveToTreasuryForm({ pixels, prompt }: { pixels: ColorPixel[]; prompt: string; }) {
    ...
    async function onSubmit(data: z.infer<typeof SaveAssetSchema>) {
     ...
     const handleDownloadObj = () => {
        ...
        a.href = url;
        const fileName = prompt.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'artwork';
    ...
    return (
        <Card className="mt-4 bg-muted/50">
            <CardHeader>
                ...
                <CardDescription>Download for 3D printing or declare this artwork as an asset.</CardDescription>
            </CardHeader>
            <CardContent>
                <div className="space-y-4">
                    <Button onClick={handleDownloadObj} variant="secondary" className="w-full">
                        <Download className="mr-2 h-4 w-4" />
                        Download .obj File for 3D Printing
                    </Button>
                    <Form {...form}>
    ...
    return (
        <main className="container mx-auto flex min-h-[80vh] items-center justify-center px-4">
            <Card className="w-full max-w-md text-center shadow-lg">
                <CardHeader>
                    <CardTitle>Join the Workshop</CardTitle>
                    <CardDescription>Log in to collaborate with others in real-time.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Button asChild>
                    <Link href="/login">
  ...
  return (
    <main className="container mx-auto max-w-4xl py-8">
       <div className="text-center mb-8">
        ...
        <p className="text-lg text-muted-foreground mt-2">A collaborative space to experiment with generative AI tools.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <div className="space-y-8">
            <Card className="shadow-lg">
                ...
                <CardDescription>
                    Enter a prompt and watch the AI bring your idea to life in a rotatable 3D space for everyone to see.
                </CardDescription>
                </CardHeader>
                <CardContent>
                <Form {...form}>
