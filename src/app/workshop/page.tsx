...
const SaveAssetSchema = z.object({
    assetName: z.string().min(2, 'Creation name must be at least 2 characters.'),
                ...
                <div className="flex -space-x-2 overflow-hidden">
                    <TooltipProvider>
                     ...
                     {(!presentUsers || presentUsers.length === 0) && (
                        <span className="text-sm text-muted-foreground self-center">You are the only soul here.</span>
                    )}
                </div>
            </CardHeader>
        </Card>
...
const convertPixelsToObj = (pixels: ColorPixel[]): string => {
    let objContent = '# 3D Point Cloud generated by the Divine Republic Altar of Creation\n';
...
function SaveToTreasuryForm({ pixels, prompt }: { pixels: ColorPixel[]; prompt: string; }) {
    ...
    async function onSubmit(data: z.infer<typeof SaveAssetSchema>) {
        if (!user) {
            toast({ variant: 'destructive', title: 'You must be logged in to save a creation.' });
            return;
        }
        setIsSaving(true);
        try {
            ...
            toast({
                ...
                description: `"${data.assetName}" has been added to your Sanctuary.`,
            });
            form.reset();
        } catch (e) {
            const message = e instanceof Error ? e.message : 'An unknown error occurred.';
            toast({ variant: 'destructive', title: 'Failed to Save Creation', description: message });
        } finally {
            setIsSaving(false);
        }
    }
    ...
    return (
        <Card className="mt-4 bg-muted/50">
            <CardHeader>
                <CardTitle className="text-lg">Export & Consecrate</CardTitle>
                <CardDescription>Download for manifestation or consecrate this creation in your Sanctuary.</CardDescription>
            </CardHeader>
            <CardContent>
                <div className="space-y-4">
                    <Button onClick={handleDownloadObj} variant="secondary" className="w-full">
                        <Download className="mr-2 h-4 w-4" />
                        Download .obj File for Manifestation
                    </Button>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4 border-t">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Creation Tithe (USD)</FormLabel>
                                        <FormControl>
  ...
  const loadAssetFromUrl = useCallback(async (assetUrl: string) => {
    ...
    try {
      ...
      if (data.pixels) {
        ...
        await updateDoc(workshopDocRef, { pixels: data.pixels, latestPrompt: 'Loaded from Sanctuary' });
    ...
    return (
        <main className="container mx-auto flex min-h-[80vh] items-center justify-center px-4">
            <Card className="w-full max-w-md text-center shadow-lg">
                <CardHeader>
                    <CardTitle>Enter the Altar of Creation</CardTitle>
                    <CardDescription>Log in to collaborate with other souls in real-time.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Button asChild>
                    <Link href="/login">
    ...
    <main className="container mx-auto max-w-4xl py-8">
       <div className="text-center mb-8">
        <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-primary flex items-center justify-center gap-3 font-headline">
          <Beaker className="w-10 h-10" /> Altar of Creation
        </h1>
        <p className="text-lg text-muted-foreground mt-2">A collaborative space to channel the divine spark.</p>
      </div>
                    ...
                    >
                        ...
                        {!isLoading && !isWorkshopLoading && !error && !pixels && (
                             <div className="flex w-full h-full flex-col gap-4 justify-center items-center text-center text-muted-foreground p-4">
                                ...
                                <p>Your divine creation will appear here. <br/> Click and drag to behold.</p>
                            </div>
                        )}
                    </div>
                </CardContent>
