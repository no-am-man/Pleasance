// src/app/community/[id]/client-page.tsx
'use client';

import { useParams, useRouter } from 'next/navigation';
import { useUser, errorEmitter, FirestorePermissionError, getFirebase } from '@/firebase';
import { doc, collection, query, orderBy, serverTimestamp, arrayUnion, arrayRemove, updateDoc, getDoc, getDocs, setDoc, onSnapshot, Unsubscribe } from 'firebase/firestore';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { LoaderCircle, AlertCircle, ArrowLeft, Bot, User, PlusCircle, Send, LogIn, Hourglass, RefreshCw, Flag, Sparkles, KanbanIcon, Banknote, BookOpen, UserX, CornerDownRight, GitBranch } from 'lucide-react';
...
export function CommunityClientPage({ serverCommunity, allCommunities, allProfiles }: { serverCommunity: Community, allCommunities: Community[], allProfiles: CommunityProfile[] }) {
  const { user, isUserLoading } = useUser();
  const { toast } = useToast();
  const { t } = useTranslation();
  const { firestore } = getFirebase();
  const id = serverCommunity.id;
  const [isGeneratingFlag, setIsGeneratingFlag] = useState(false);

  const [community, setCommunity] = useState<Community>(serverCommunity);
  ...
  // Real-time listener for the main community document
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribeJoinRequest = onSnapshot(requestDocRef, (docSnap) => {
  ...
  }, [id, user, isUserLoading, firestore, allCommunities]);
                        ...
                        <CardFooter>
                            <AlertDialog>
                                <AlertDialogTrigger asChild>
                                    <Button variant="destructive"><UserX className="mr-2 h-4 w-4" />{t('community_page_leave_button')}</Button>
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    <AlertDialogHeader>
